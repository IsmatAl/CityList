...
public class DataPopulator {
    ...
    private List<City> loadDataFromCSV(String fileName) throws IOException, CsvException {
        ...
        try (final CSVReaderHeaderAware csvFileReader = new CSVReaderHeaderAware(new BufferedReader(new InputStreamReader(lastNamesFile.getInputStream())))) {
            final List<String[]> allRows = csvFileReader.readAll();
            return allRows.stream()
                    .map(v -> City.builder().cityName(v[1]).imageURL(v[2]).build())
