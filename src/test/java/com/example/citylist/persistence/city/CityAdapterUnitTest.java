...
public class CityAdapterUnitTest {
    ...
    void shouldUpdateCity() {
        ...
        assertThat(cityEntityArgumentCaptor.getValue()).usingRecursiveComparison().ignoringFields("id",
                "imageURL", "createdAt", "createdBy", "modifiedAt", "modifiedBy").isEqualTo(cityWithNewName);
    }
    ...
    void shouldReturnCities() {
        ...
        given(cityEntityRepository.findCities(cityRequestFilter.getCreatedFrom(), cityRequestFilter.getCityName(), pageRequest))
                .willReturn(new PageImpl<>(List.of(city), pageRequest, 1));

        // when
        final Page<City> actual = cityAdapter.getAll(pageRequest, cityRequestFilter);

        // then
        verify(cityEntityRepository).findCities(cityRequestFilter.getCreatedFrom(),
                cityRequestFilter.getCityName(),
                PageRequest.of(1, 3));
