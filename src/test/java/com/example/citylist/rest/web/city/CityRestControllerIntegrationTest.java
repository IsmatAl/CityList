...
public class CityRestControllerIntegrationTest {
    ...
    void shouldGetCitiesWithCustomPagination() throws Exception {
        ...
        final CityRequestFilter cityRequestFilter = cityRequestFilterBuilderWithNoFilter()
                .build();
        given(dateService.convertStringToInstant(null)).willReturn(null);
        given(cityService.getAll(PageRequest.of(firstPage, pageSize),
                cityRequestFilter))
                .willReturn(new PageImpl<>(List.of(city1), PageRequest.of(firstPage, pageSize), 2));
        given(cityService.getAll(PageRequest.of(secondPage, pageSize),
                cityRequestFilter))
                .willReturn(new PageImpl<>(List.of(city2), PageRequest.of(secondPage, pageSize), 2));
        given(cityResponseResourceConverter.convertToResource(city1))
                .willReturn(cityResponseResource1);
        given(cityResponseResourceConverter.convertToResource(city2))
                .willReturn(cityResponseResource2);
        ...
        verify(cityService).getAll(PageRequest.of(firstPage, pageSize),
                cityRequestFilter);
        verify(cityService).getAll(PageRequest.of(secondPage, pageSize),
                cityRequestFilter);
